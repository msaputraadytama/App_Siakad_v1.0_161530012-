...
public class IfrUsers extends javax.swing.JInternalFrame {
    ...
    String[] KeyLev;
    private void listLevel(){//method agar combobox menjadi otomatis/dinamis
        try{
            _Cnn = null;
            _Cnn = getCnn.getConnection();
            ...
            Statement stat = _Cnn.createStatement();
            ResultSet res = stat.executeQuery(sqlselect);
            cmbLevUser.removeAllItems();
            cmbLevUser.repaint();
            cmbLevUser.addItem("-- Pilih --");
            int i = 1;
            while(res.next()){
                cmbLevUser.addItem(res.getString("lev_user"));
                i++;
            }
            
            res.first();
            KeyLev = new String[i+1];   //mengatur primary key yg disimpan pd tbuser
            for(Integer x = 1; x<i; x++){
                KeyLev[x] = res.getString(1);//res.getString(1) --> res.getString("id_user")
                res.next();//
            }
            
        }catch(SQLException ex){
            JOptionPane.showMessageDialog(this, "Error method listLevel() : " + ex);
        }
